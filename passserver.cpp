...
///addtional public member functions
size_t PassServer::cap()const {return server.cap();}

///given public member functions to define
...
bool PassServer::addUser(std::pair<std::string,  std::string> & kv){
    auto x = kv;
    //std::cout <<"in addUser\n";
    x.second = encrypt(x.second);
    //std::cout <<"x.second : "<< x.second <<'\n';
    return server.insert(x);}

//: move version of addUser.
bool PassServer::addUser(std::pair<std::string, std::string> && kv){
    auto x = kv;
    //std::cout <<"in addUser\n";
    x.second = std::move(encrypt(x.second));
    //std::cout <<"x.second : "<< x.second <<'\n';
    return server.insert(x);
}

//: delete an existing user with username k.
bool PassServer::removeUser(const std::string & k){ return server.remove(k);}
...
bool PassServer::changePassword(const std::pair<std::string, std::string> &p, const std::string & newpassword){
    ...
    if(server.match(temp) && (temp.second != encrypt(newpassword))){
...
bool PassServer::find(const std::string & user){ return server.contains(user);}
...
void PassServer::dump()const { server.dump();}

//: return the size of the HashTable (the number of username/password
// pairs in the table).
size_t PassServer::size()const {return server.size();}
...
bool PassServer::write_to_file(const char *filename)const{ return server.write_to_file(filename);}
...
std::string PassServer::encrypt(const std::string & str)const{
    char salt[] = "$1$########";
    std::string c =crypt(str.c_str(), salt);
    //std::cout<< "in encrypt result: " << c.substr(c.find_last_of("$")+1)<<'\n';
    return c.substr(c.find_last_of("$")+1);;
}